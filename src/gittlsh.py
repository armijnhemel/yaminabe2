...
import os, os.path, sys, re, subprocess, shutil, hashlib, sqlite3, cPickle, copy
import multiprocessing, ConfigParser, collections
...
def writeresults(reportqueue, gitdatabase, project, giturl, sha256seendict, patchiddict):
	...
	while True:
		...
		if results == []:
			## record revisions that are not relevant, for example revisions
			## in which there is no interesting content or where just some metadata
			## (permissions, etc.) were changed.
			if hascontent:
			...
			for result in results:
				...
				cursor.execute('insert into tlshentries (filepath, endindex, sha256sum, tlsh, filename, project, gitrevision, revisiontype, giturl) values (?,?,?,?,?,?,?,?,?)', (filename, endindex, sha256sum, tlshhash, os.path.basename(filename), project, gitrevision, revisiontype, giturl))
			...
			for patchid in patchids:
				if mergecommit:
					if patchid in patchiddict:
						continue
				cursor.execute('insert into patchids (patchid, gitrevision, giturl) values (?,?,?)', (patchid, gitrevision, giturl))
...
def gitscan(scanqueue, reportqueue, gitdir, lock, seendict, gitpath):
	...
	while True:
		...
		p = subprocess.Popen([gitpath, 'show', '--pretty=oneline', '--numstat', gitrevision], cwd=gitdir, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		...
		mergecommit = False
		p = subprocess.Popen([gitpath, 'show', '-s', gitrevision], cwd=gitdir, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		(stanout, stanerr) = p.communicate()
		for s in stanout.split('\n'):
		...
		p = subprocess.Popen([gitpath, 'show', '--pretty=oneline', '--numstat', '-m', gitrevision], cwd=gitdir, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		(stanout, stanerr) = p.communicate()
		newtotalfiles = 0
		for s in stanout.split('\n')[1:]:
			blameres = numstatre.match(s)
		...
		if len(filternames) != 0:
			...
			for filtername in filternames:
				if stopprocessing:
					break
				if relative:
					p = subprocess.Popen([gitpath, 'show', '-m', gitrevision, '--relative=%s' % filtername], cwd=gitdir, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
				else:
					p = subprocess.Popen([gitpath, 'show', '-m', gitrevision], cwd=gitdir, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def main(argv):
	...
	if options.cfg != None:
		try:
			...
			print >>sys.stderr, "path '%s' does not exist. Exiting." % options.cfg
			sys.exit(1)
	else:
	...
	for section in config.sections():
		...
		try:
			...
			print >>sys.stderr, "Database for writing Git results not specified in configuration. Exiting."
			configfile.close()
			sys.exit(1)
		try:
			optimizedb = False
	...
	if not os.path.exists(os.path.dirname(os.path.normpath(gitdatabase))):
		print >>sys.stderr, "Directory for storing database does not exist. Exiting."
		sys.exit(1)
	...
	for section in config.sections():
		...
		if configtype == 'project':
			...
			try:
				...
				print >>sys.stderr, "Git directories not specified in configuration. Skipping."
				continue
			giturl = None
			try:
				giturl = config.get(section, 'giturl')
			except:
				print >>sys.stderr, "Git URL in %s not provided, extracting from source." % section
			try:
				project = config.get(section, 'project')
			except:
				print >>sys.stderr, "Project name not specified in configuration. Skipping."
				continue
			try:
	...
	if len(projects) == 0:
		print >>sys.stderr, "No valid projects for scanning found. Exiting."
		sys.exit(1)

	if filter(lambda x: x[-2] == True, projects) == []:
		print >>sys.stderr, "No projects for scanning found. Exiting."
		sys.exit(1)
	...
	for pr in projects:
			...
			for gc in gitconfiglines:
				if 'url =' in gc:
					...
					if giturl == None:
						...
						if giturl != gu:
							print >>sys.stderr, "config url %s for %s does not match %s" % (gu, g, giturl)
							exitgit = True
							continue
		...
		if exitgit:
			if origgiturl != None:
				giturltopriority[origgiturl] = (project, priority)
			print >>sys.stderr, "Errors found in Git directories for project %s, continuing with configuration for next project" % project
			continue
		...
		if len(gitdirs) > 1:
			for g in set(gitdirs):
				...
				if headrevision == None:
					headrevision = githeadrevision
				if headrevision != githeadrevision:
					print >>sys.stderr, "directory %s does not match HEAD revision %s" % (g, headrevision)
					exitgit = True
	...
	for pr in newprojects:
		...
		(stanout, stanerr) = p.communicate()
		gitrevisions = stanout.strip().split('\n')
		...
		gitrevisions = filter(lambda x: x not in ignorerevisions, gitrevisions)
		...
		if restorestate:
			if os.path.exists(statefile):
				...
				(dumpgiturl, dumpdict, dumpsha256seendict, dumppatchiddict) = cPickle.load(statepicklefile)
		...
		for i in range(0,processamount):
			p = multiprocessing.Process(target=gitscan, args=(scanqueue,reportqueue,gitdirs[i], lock, seendict, gitpath))
			processpool.append(p)

		r = multiprocessing.Process(target=writeresults, args=(reportqueue, gitdatabase, project, giturl, sha256seendict, patchiddict))
		...
		## clean up the database. Search for files that are the same and which occur in both
		## merge commits and non-merge commits. Favour non-merge commits. Leave non-merge commits
		## alone.
		...
		if optimizedb:
			...
			## now check if the there is still data left for the revisions. If not, add it to the
			## deleted revisions list
			for gitrevision in deletecandidates:
				res = cursor.execute("select * from tlshentries where gitrevision=? LIMIT 1", (gitrevision,)).fetchall()
				if res != []:
					continue
				res = cursor.execute("select * from emptyrevisions where gitrevision=? LIMIT 1", (gitrevision,)).fetchall()
				if res != []:
					continue
				cursor.execute('insert into deletedrevisions (gitrevision, project, giturl) values (?,?,?)', (gitrevision, project, giturl))
				conn.commit()
		...
		if restorestate:
			...
			cPickle.dump((giturl, dumpdict, dumpsha256dict, dumppatchiddict), statepicklefile)
			statepicklefile.close()
